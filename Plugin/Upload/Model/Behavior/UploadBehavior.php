		...
		foreach ($this->settings[$model->alias] as $field => $options) {
			foreach ($results as & $result) {
				if ( ! empty($result[$model->alias][$field])) {
					$result[$model->alias]['__'.$field] = str_replace(array($options['rootDir'], WEBROOT_DIR, DS), array('', '', '/'), $options['path']);

					if ('_getPathPrimaryKey' === $options['pathMethod']) {
						$result[$model->alias]['__'.$field] .= $result[$model->alias]['id'].'/';
					}

					$result[$model->alias]['__'.$field] .= $result[$model->alias][$field];
				}
			}
			unset($result);
		}
