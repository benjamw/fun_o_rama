...
var PLURALS = inflector.plurals,
	SINGULARS = inflector.singulars,
	UNCOUNTABLES = inflector.uncountables,
	HUMANS = inflector.humans;
...
var plural = function (rule, replacement) {
	//inflector.uncountables.delete(rule) if rule.is_a?(String)
	//inflector.uncountables.delete(replacement)
...
var irregular = function (s, p) {
	//inflector.uncountables.delete(singular);
	//inflector.uncountables.delete(plural);
...
var human = function (rule, replacement) {
	//inflector.uncountables.delete(rule) if rule.is_a?(String)
	//inflector.uncountables.delete(replacement)
	inflector.humans.push([rule, replacement]);
}
...
inflector.underscore = function (word) {
	return this.split(word, "_").toLowerCase();
}
...
inflector.dash = inflector.dasherize = function (word) {
	return this.split(word, "-").toLowerCase();
}

/**
 * Converts a CamelCase word to table format (underscore, plural).
 */
inflector.tableize = function (word) {
	word = this.split(word, "_").toLowerCase();
	return this.pluralize(word);
}

/**
 * Converts a under_scored word to class format (CamelCase, singular).
 */
inflector.classify = function (word) {
	word = this.singularize(word);
	return this.camelcase(word);
}

/**
 * Converts a under_scored word to CamelCase format.
 */
inflector.camelcase = function (word) {
	var words = word.split("_");
	word = '';

	for (var i in words) {
		word += words[i].ucFirst( );
	}
