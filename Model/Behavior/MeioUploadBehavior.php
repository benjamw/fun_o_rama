...
// put this in your model to fix any failed afterFinds
	...
	function _fix_upload_path($results) {
 ...
 * @access protected
 */
	function _deleteFilesList(Model $model) {
 ...
 */
	...
	/**
	 * Appends the path data to the filename stored in the fieldName
	 *
	...
	function afterFind(Model $model, $results, $primary = false) {
		...
		if (count($results)) {
			foreach ($results as $key => $value) {
				foreach ($this->__fields[$model->alias] as $fieldName => $options) {
					if (isset($results[$key][$model->alias]) && isset($value[$model->alias][$fieldName])) {
						$results[$key][$model->alias][$fieldName.'_path'] = str_replace('\\', '/', $this->_replaceTokens($model, $options['dir'], $fieldName)).'/'.urlencode($value[$model->alias][$fieldName]);
					}
					elseif (isset($results[$key][$model->alias][0])) {
						foreach ($results[$key][$model->alias] as $a_key => $assoc_value) {
							$results[$key][$model->alias][$a_key][$fieldName.'_path'] = str_replace('\\', '/', $this->_replaceTokens($model, $options['dir'], $fieldName)).'/'.urlencode($value[$model->alias][$a_key][$fieldName]);
						}
					}
					elseif (isset($value[$fieldName])) {
						$results[$key][$fieldName.'_path'] = str_replace('\\', '/', $this->_replaceTokens($model, $options['dir'], $fieldName)).'/'.urlencode($value[$fieldName]);
					}
				}
			}
		}
 ...
 * @access public
 */
	function upload(Model $model, $data) {
 ...
 * @access public
 */
	function uploadCheckMaxWidth(Model $model, $data) {
 ...
 * @access public
 */
	function uploadCheckHttpPost(Model $model, $data) {
 ...
 * @access protected
 */
	function _uploadFile(Model $model, $data = null) {
 ...
 * @access protected
 */
	function _createThumbnails(Model $model, $data, $fieldName, $saveAs, $ext, $options) {
 ...
 * @access protected
 */
	function _replaceTokens(Model $model, $string, $fieldName, $tokens = array()) {
		return str_replace(
			$tokens,
 ...
 * @access protected
 */
	function _setFileToRemove(Model $model, $fieldName, $thumbDir) {
 ...
 * @access protected
 */
	function _markForDeletion(Model $model, $fieldName, $data, $default, $thumbDir) {
		if (!empty($data[$model->alias][$fieldName]['remove'])) {
			if ($default) {
 ...
 * @access protected
 */
	function _deleteFiles(Model $model, $field, $filename, $dir) {
 ...
 * @access protected
 */
	function _nullifyDataFields(Model $model, $fieldName, $data, $options) {
		$data[$model->alias][$fieldName] = null;
